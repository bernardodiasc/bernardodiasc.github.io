...
The blueprint theme relates to things I love to do, as in sketching UI, wireframing layout and information and front end architecture. Bootstraping projects is really fun. So I'm intended to explore a lot these subjects here in this blog, this theme kinda of made sense to me I hope it sticks. 😄

On more technical details, I've created a lot CSS Variables to store constant values for typography, as colours and font-faces. During design updates I did a lot tries and updating directly on CSS variables made this process much simpler.

I also created the `<TextBlock />` [(see example on docs)](https://bernardodiasc.github.io/docs/?selectedKind=Displays%2FTextBlock&selectedStory=Example) component to hold all the typography styles within. I'll dedicate one post just to explain why I isolate typography styles from global CSS scope, for now only the basic typography styles is set on `body` and `h1-6`.
