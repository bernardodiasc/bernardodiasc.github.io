...
This is the taxonomy I landed on. It isn't the only one that would work. It is the one that emerged from actual use, after enough mistakes that the categories felt obvious by the time I named them.
...
QA skills are tester-facing. They run a scenario, capture the output, and either compare against a known good baseline or hand the output back for human inspection. They tend to be invoked after a change, not during it.

The line between `qa` and `debug` can blur. The rough rule I use: a debug skill answers *what's happening?* on live state; a QA skill answers *did the thing I changed break what was already working?* on test scenarios.
...
The mental model: typed skills are *for the agent to do a thing in the project*. Integration skills are *infrastructure*. Different shapes for different purposes.
...
**A `meta` type for skills that operate on other skills.** Sounded clever. Was solving a problem nobody had. The skills I'd been considering meta were actually `utils` (testing a skill) or `guide` (explaining how to write a new skill). Naming them properly removed the need for the type.
...
- [Building debug skills that inspect live systems](/2026/04/27/building-debug-skills-that-inspect-live-systems)
- [Subagents — specialized AI that doesn't pollute your context](/2026/04/28/subagents-specialized-ai-that-doesnt-pollute-your-context)
