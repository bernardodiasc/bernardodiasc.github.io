...
The three modes together let you build a rule library where each rule is loaded **only when it's useful**. That sounds obvious. It's not what most projects do. Routed rules are the middle path between one giant always-on file and making the agent reinvent project conventions every session.
...
The short version before the next posts go deeper:
