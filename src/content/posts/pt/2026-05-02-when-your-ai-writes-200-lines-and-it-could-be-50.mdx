...
O check funciona porque engenheiros sêniores — do tipo que mantiveram codebases ao longo de anos — têm uma alergia forte a código que faz mais do que o momento exige. Eles já viram helpers abstraídos com elegância se calcificarem em legado. Já debugaram testes que mockam tudo. Sabem que 50 linhas que qualquer um consegue ler vencem 200 linhas que só o autor original consegue estender.
...
Um pequeno, mas que se acumula. O agente tem uma tendência a adicionar comentários que simplesmente reafirmam o que a próxima linha de código faz.
...
Todo comentário é verdadeiro. Todo comentário também é redundante. Um leitor desse código consegue ver que `with open(path) as f` abre o arquivo de configuração. Dizer de novo num comentário adiciona linhas, não entendimento.

A simplificação: apenas remova os comentários.
...
A regra, no arquivo de rule always-on: *não adicione comentários que apenas narram o que o código faz.* Comentários devem explicar *por que*, não *o que*. O *o que* está ali mesmo no código. O *por que* — a restrição que não é visível no código, o tradeoff que foi considerado, a pegadinha que pegou alguém uma vez — é para isso que comentários servem.

Code review sênior consistentemente marca comentários narrativos como ruído. A rule codifica esse sinal para que o agente não os gere em primeiro lugar.
...
Esse padrão é difícil de pegar porque cada adição individual parece útil. A correção é a rule de mudanças cirúrgicas do post anterior: *cada linha alterada deve rastrear diretamente ao pedido do usuário.* Linhas que não rastreiam de volta ao pedido — mesmo boas linhas, mesmo linhas bem-intencionadas — não pertencem ao diff.

## Como é o pushback na prática

O arquivo de rule é necessário mas não suficiente. Mesmo com a rule carregada, o agente às vezes vai produzir output com overengineering, especialmente na primeira passada. O trabalho restante é meu: eu tenho que realmente fazer pushback quando vejo um desses padrões.

Como o pushback se parece, na prática, é curto e direto.
...
O agente sempre obedece. A próxima iteração é sempre mais enxuta. O custo de uma rodada de pushback é pequeno. O custo de aceitar a versão com overengineering é que você agora fez o commit, e remover depois é mais caro do que recusar adicionar agora.
...
- [Guardrails comportamentais para assistentes de IA de programação](/pt/2026/05/01/behavioral-guardrails-for-ai-coding-assistants)
