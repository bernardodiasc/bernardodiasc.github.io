...
Esse número não é uma meta. É onde eu acabei chegando no projeto em que estava trabalhando, e olhando pra trás, faz sentido.

O projeto tem algumas superfícies naturais:

- **Estado ao vivo** — o que o agente está pensando no momento. Skills para inspecionar checkpoints, decodificar estado do grafo, ler a thread de mensagens que o agente está respondendo.
- **Configuração / cache** — o que já foi computado antes. Skills para ler e limpar avaliações em cache, consultar rate limits, verificar timing.
- **Sistemas externos** — as plataformas de terceiros com as quais o agente se comunica. Skills para buscar um registro por ID, executar uma query de busca, postar uma mensagem de volta.
- **Procedimentos internos** — mudanças multi-step repetíveis. Skills que guiam o agente por "adicionar um novo filtro de busca" ou "conectar um novo intent pelo pipeline."
- **Quality assurance** — replay de cenários, captura de outputs, verificações de regressão.

Cada superfície produz algo entre três e dez skills conforme o projeto amadurece. Multiplique por cinco superfícies e você chega em algum lugar entre vinte e poucos e trinta e poucos. Isso não é mágica — é a forma natural de um sistema não trivial que uma pessoa mais um agente de IA opera.
...
Uma **rule** carrega automaticamente quando você está trabalhando num arquivo correspondente. É ambient. O agente lê quer você peça ou não. Rules servem para orientação de estado estável: voz, convenções, políticas de segurança, coisas que devem sempre moldar o comportamento do agente num determinado tipo de arquivo.
...
Provavelmente não. Provavelmente você deveria ter quantas o seu projeto produzir naturalmente, escritas uma de cada vez, com o limiar que descrevi acima: *se faço isso duas vezes, capturo.*

Alguns projetos são bem servidos por cinco skills. Outros por cinquenta. O número é consequência da complexidade do sistema e de quão frequentemente você opera nele. Contar suas skills é mais ou menos tão informativo quanto contar seus casos de teste — útil como cheiro, não como meta.

O padrão que se sustenta entre tamanhos de projeto é o mais simples: quando um agente de código com IA opera no mesmo projeto repetidamente, **a memória dele sobre o projeto acaba no seu repositório**. Parte dessa memória acaba em rules, que direcionam comportamento em edições de arquivo. O resto acaba em skills, que permitem ao agente executar procedimentos sob demanda. Ambos se acumulam. Ambos são baratos de escrever na primeira vez, e a maioria deles continua se pagando enquanto o projeto viver.
...
- [Rules como memória do projeto](/pt/2026/04/24/rules-as-project-memory)
