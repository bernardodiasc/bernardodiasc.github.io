...
A abordagem ingênua é ter um conjunto de arquivos na pasta de uma ferramenta e aceitar que trocar de ferramenta significa reescrever tudo. É assim que a maioria dos projetos começa.

A abordagem um pouco menos ingênua é manter *duas* cópias — uma em `.cursor/`, uma em `.claude/` — e mantê-las sincronizadas manualmente. Isso funciona para umas três skills antes de desmoronar. Na décima skill, as duas pastas divergiram, e você está gastando mais tempo reconciliando do que escrevendo.

A abordagem realmente boa é a que vou descrever: escolha um local de fonte-da-verdade, e faça o diretório esperado de cada ferramenta apontar via symlink para ele. Uma edição de arquivo, toda ferramenta vê a mudança.
...
**`src/` é genérico.** Não tem nome de nenhum coding agent específico. Se uma nova ferramenta aparecer que espera sua config em `.someother/`, eu adiciono mais três symlinks apontando para `src/` e a nova ferramenta começa a funcionar sem mover nenhum arquivo.

**`src/` fica por conta própria na raiz do projeto.** Fica como irmão das pastas específicas de ferramenta, não dentro de nenhuma delas. Isso é proposital. Se eu deletasse `.cursor/` amanhã — digamos que saí do Cursor — `src/` não é afetado. O conteúdo canônico não tem dependência da existência do diretório de nenhuma ferramenta.
...
**Padrões glob podem variar.** Os matchers de glob de algumas ferramentas se comportam diferentemente em edge cases (caminhos profundamente aninhados, arquivos ocultos). Quando isso importa, escrevo o glob na forma mais ampla e aceito leve overmatching em uma ferramenta.
...
**Experimentos entre ferramentas se tornam possíveis.** Às vezes quero comparar como a mesma skill performa em dois agents diferentes. Com esse layout, "a mesma skill" é *literalmente* o mesmo arquivo. Qualquer diferença de comportamento é do modelo ou do runtime, não de drift na skill.
