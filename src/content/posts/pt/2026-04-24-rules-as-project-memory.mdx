...
Vou escrever este post no formato `.mdc` do Cursor porque é o que eu uso. Os padrões se transferem. Se você está numa ferramenta diferente, os arquivos equivalentes vão parecer ligeiramente diferentes, mas o raciocínio é o mesmo.
...
Os três modos juntos permitem construir uma biblioteca de rules onde cada rule é carregada **apenas quando é útil**. Parece óbvio. Não é o que a maioria dos projetos faz. A maioria dos projetos ou joga tudo num único arquivo gigante sempre ativo (o que custa tokens em cada request e deixa as instruções do agente confusas) ou pula rules inteiramente (o que deixa o agente reinventando as convenções do projeto toda sessão). Rules roteadas são o caminho do meio.
...
- **Estrutura obrigatória.** "Todo post de blog deve terminar com uma seção `## Further reading`." Sem a rule, o agente esquece uma em cada três vezes. Com ela, o agente não esquece.
...
## Rules e skills, em uma frase

Algumas pessoas vão ler isso e perguntar: qual a diferença entre uma rule e uma "agent skill" ou um "subagent"? Ambos vão ter seus próprios posts. A versão bem curta:
...
Você normalmente vai precisar dos três. Rules são a fundação porque são baratas e ambiente. Os outros dois entram em cena quando você supera o que rules sozinhas conseguem fazer.
...
- [AI agent skills — por que escrevi 33](/pt/2026/04/25/ai-agent-skills-why-i-wrote-33-of-them)
- [Guardrails de comportamento para assistentes de código com IA](/pt/2026/05/01/behavioral-guardrails-for-ai-coding-assistants)
