...
You'll usually want all three on a non-trivial project: rules raise the floor, skills give procedures, and subagents handle the heavy specialized work the parent shouldn't keep in its own context.

## A small piece of advice

If you've never written a subagent, start with the security-review shape: read-only, narrow specialization, structured output. Tomorrow's post walks through the one I actually use.

Once you've felt how that's different from doing the same review in the parent's context, you'll start spotting other tasks that fit the same shape.
